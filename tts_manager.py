...
class TTSManager:
    ...
    def load_cached_voices(self) -> None:
        ...
        if not self.cache_path or not os.path.exists(self.cache_path):
            # Provide static Google presets so the UI shows real voices even without a cache.
            if self.provider.lower() == "google":
                self._apply_google_fallback()
            return
        try:
            ...
            if isinstance(data, list):
                ...
                for v in data:
                    if not (isinstance(v, dict) and v.get("name")):
                        continue
                    if self.provider.lower() == "google" and "-" not in str(v.get("name")):
                        # Skip bogus entries like "Achernar" that the SDK cannot synthesize by name.
                        continue
                    filtered.append(v)
                self._voices = filtered
        except Exception as exc:
            print(f"[TTS] WARN: Failed to load cache '{self.cache_path}': {exc}")
        if not self._voices and self.provider.lower() == "google":
            self._apply_google_fallback()
        ...
        if provider == "google":
            if self.google_sdk_enabled:
                try:
                    ...
                    self.last_error = f"Google SDK synth failed: {exc}"
                    print(f"[TTS] WARN: {self.last_error}")
            # Placeholder: no REST fallback implemented here
            if self.last_error is None:
                self.last_error = "Google TTS not available (SDK disabled or missing)."
            return None
            ...
            self.last_error = "ElevenLabs provider not implemented."
            return None
        return None
    ...
    def fetch_live_google_voices(self) -> bool:
        ...
        try:
            ...
            # Only keep voices with canonical Google-style names (contain a dash) to avoid invalid placeholders.
            normalized = []
            for v in voices or []:
                name = v.get("name")
            ...
            self.last_error = f"Live Google voices fetch failed: {exc}"
            print(f"[TTS] WARN: {self.last_error}")
            self._apply_google_fallback()
        return False
    ...
    def _apply_google_fallback(self) -> None:
        """Populate voices with a static set of real Google TTS voices."""
        try:
            from google_tts_live import FALLBACK_VOICES  # type: ignore
        except Exception:
            self.last_error = "Fallback voices unavailable."
            return
        normalized = []
        ...
        try:
            ...
            print(f"[TTS] WARN: Failed to save fallback voices cache: {exc}")
