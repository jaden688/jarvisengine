...
def build_system_prompt(
    ...
    default_rhythm_notes = {
        ...
        "twitch": "Quick, jumpy cadence with sparks of creativity; keep it concise.",
        "cascade": "Flowing, connected thoughts; weave ideas smoothly.",
        "stutter": "Cautious, fragmented delivery; ask clarifying questions.",
        "burst": "High-energy, punchy responses; still remain accurate.",
    }
    rhythm_notes = default_rhythm_notes.get(rhythm_mode, "Default flip/flop rhythm cadence.")
    ...
    if safety_on:
        safety_block = (
            ...
            "but keep your persona tone and do not over-police casual/friendly language.\n"
        )
    else:
        safety_block = (
            "SAFETY MODE: OFF\n"
...
class JLEngineApp:
    def __init__(self, root, safety_level="full"):
        ...
        self.tab_cnc = ttk.Frame(self.notebook, style="App.TFrame")
        ...
        self.notebook.add(self.tab_cnc, text="CNC Control")
        ...
        # CNC control + diagnostics build
        self._build_cnc_tab(self.tab_cnc)
        ...
        self.last_signals = self.signal_scorer.score("")  # seed HUD with baseline signals
    ...
    def _build_control_panels(self, parent):
        ...
        # Default safety to OFF; user can toggle on as needed.
        self.safety_var = StringVar(value="OFF")
        self.safety_button = ttk.Button(frame, text="Safety: OFF", command=self.toggle_safety)
        ...
        self.play_last_console_button = ttk.Button(frame, text="Play Last", command=self.play_last_audio)
        self.play_last_console_button.grid(row=1, column=6, sticky="w", padx=5, pady=4)
    ...
    def _build_hud_summary(self, parent):
        ...
        add_row(7, "Sentiment:", "sentiment")
        add_row(8, "Arousal:", "arousal")
        add_row(9, "Confusion:", "confusion")
        add_row(10, "Pace:", "pace")
        add_row(11, "Memory Density:", "memory_density")
        add_row(12, "Directive:", "directive")
        add_row(13, "Backend:", "backend_name")
        add_row(14, "Model:", "model_name")
        add_row(15, "Latency (ms):", "latency_ms")
        add_row(16, "Safety:", "safety")
    ...
    def _build_cnc_tab(self, parent):
        """CNC control panel using the OI cnc tool bridge."""
        wrap = ttk.Frame(parent, style="HUD.TFrame")
        wrap.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        ttk.Label(wrap, text="CNC Control (COM4)", style="Header.TLabel").pack(anchor="w", pady=(0, 6))
        ttk.Label(wrap, text="Requires Tools: ON. Close Candle so COM4 is free.", style="TLabel").pack(anchor="w", pady=(0, 4))

        ctrl = ttk.Frame(wrap, style="HUD.TFrame", borderwidth=1, relief="solid")
        ctrl.pack(fill=tk.X, padx=5, pady=6)

        # Spindle controls
        spin_frame = ttk.Frame(ctrl, style="HUD.TFrame")
        spin_frame.grid(row=0, column=0, sticky="w", padx=6, pady=6)
        ttk.Label(spin_frame, text="Spindle speed:", style="TLabel").grid(row=0, column=0, sticky="w", padx=2, pady=2)
        self.cnc_speed = tk.StringVar(value="500")
        ttk.Entry(spin_frame, textvariable=self.cnc_speed, width=8).grid(row=0, column=1, sticky="w", padx=2, pady=2)
        ttk.Button(spin_frame, text="On", command=lambda: self._send_cnc({"action": "spindle_on"})).grid(row=0, column=2, padx=4, pady=2)
        ttk.Button(spin_frame, text="Off", command=lambda: self._send_cnc({"action": "spindle_off"})).grid(row=0, column=3, padx=4, pady=2)

        # Jog controls
        jog_frame = ttk.Frame(ctrl, style="HUD.TFrame")
        jog_frame.grid(row=0, column=1, sticky="w", padx=6, pady=6)
        ttk.Label(jog_frame, text="Jog (mm) | Feed:", style="TLabel").grid(row=0, column=0, sticky="w", padx=2, pady=2)
        self.cnc_dx = tk.StringVar(value="1.0")
        self.cnc_dy = tk.StringVar(value="0.0")
        self.cnc_dz = tk.StringVar(value="0.0")
        self.cnc_feed = tk.StringVar(value="200")
        ttk.Entry(jog_frame, textvariable=self.cnc_dx, width=6).grid(row=1, column=0, padx=2, pady=2)
        ttk.Entry(jog_frame, textvariable=self.cnc_dy, width=6).grid(row=1, column=1, padx=2, pady=2)
        ttk.Entry(jog_frame, textvariable=self.cnc_dz, width=6).grid(row=1, column=2, padx=2, pady=2)
        ttk.Entry(jog_frame, textvariable=self.cnc_feed, width=8).grid(row=1, column=3, padx=6, pady=2)
        ttk.Button(jog_frame, text="Send Jog", command=lambda: self._send_cnc({"action": "jog"})).grid(row=1, column=4, padx=4, pady=2)

        # Status + unlock row
        status_frame = ttk.Frame(ctrl, style="HUD.TFrame")
        status_frame.grid(row=0, column=2, sticky="w", padx=6, pady=6)
        ttk.Button(status_frame, text="Status ?", command=lambda: self._send_cnc({"action": "status"})).grid(row=0, column=0, padx=4, pady=2)
        ttk.Button(status_frame, text="Unlock ($X)", command=lambda: self._send_cnc({"action": "unlock"})).grid(row=0, column=1, padx=4, pady=2)

        # Custom CNC payload sender
        custom_frame = ttk.Frame(wrap, style="HUD.TFrame")
        custom_frame.pack(fill=tk.X, padx=5, pady=4)
        ttk.Label(custom_frame, text="Custom CNC payload (JSON for tool):", style="TLabel").grid(row=0, column=0, sticky="w", padx=2, pady=2)
        self.cnc_custom_input = tk.StringVar()
        custom_entry = ttk.Entry(custom_frame, textvariable=self.cnc_custom_input)
        custom_entry.grid(row=1, column=0, sticky="ew", padx=2, pady=2)
        custom_entry.bind("<Return>", lambda e: self._send_cnc_custom(self.cnc_custom_input.get()))
        ttk.Button(custom_frame, text="Send", command=lambda: self._send_cnc_custom(self.cnc_custom_input.get())).grid(row=1, column=1, sticky="w", padx=4, pady=2)
        ...
        ttk.Label(log_frame, text="CNC Log", style="TLabel").pack(anchor="w", padx=5, pady=4)
        self.cnc_log = scrolledtext.ScrolledText(
            log_frame,
            wrap=tk.WORD,
            state="disabled",
            font=("Consolas", 10),
        ...
        self.cnc_log.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

    def _append_cnc_log(self, text: str):
        if not hasattr(self, "cnc_log"):
            return
        self.cnc_log.configure(state="normal")
        self.cnc_log.insert(tk.END, text + "\n")
        self.cnc_log.see(tk.END)
        self.cnc_log.configure(state="disabled")

    def _send_cnc(self, payload: dict):
        """Send a CNC tool request through the interpreter backend."""
        if not self.tool_enabled:
            self._append_cnc_log("[SYSTEM] Tools are OFF. Toggle Tools ON.")
            return

        action = payload.get("action")
        tool_payload = {"mode": "raw"}
        ...
        if action == "status":
            ...
            tool_payload["gcode"] = f"M3 S{self.cnc_speed.get()}"
        elif action == "spindle_off":
            tool_payload["gcode"] = "M5"
        elif action == "jog":
            try:
                dx_f = float(self.cnc_dx.get())
                dy_f = float(self.cnc_dy.get())
                dz_f = float(self.cnc_dz.get())
                feed_f = float(self.cnc_feed.get())
            except ValueError as exc:
                self._append_cnc_log(f"[SYSTEM] Invalid jog values: {exc}")
                return
            dx = str(dx_f)
            dy = str(dy_f)
            dz = str(dz_f)
            ...
            move_parts = []
            if dx_f != 0.0:
                move_parts.append(f"X{dx}")
            if dy_f != 0.0:
                move_parts.append(f"Y{dy}")
            if dz_f != 0.0:
                move_parts.append(f"Z{dz}")
            move_line = f"G0 {' '.join(move_parts)} F{feed}" if move_parts else ""
        ...
        message = {"role": "user", "content": json.dumps({"mode": "tool", "tool": "cnc", "payload": tool_payload})}
        ...
        try:
            ...
            if isinstance(result, tuple) and len(result) == 2:
                ...
                response_text, meta = result, {}
            self._append_cnc_log(str(response_text))
            if meta:
                self._append_cnc_log(str(meta))
        except Exception as exc:
            self._append_cnc_log(f"[SYSTEM] Tool error: {exc}")

    def _send_cnc_custom(self, raw_text: str, log_fn=None):
        """Send a raw CNC tool payload (JSON) through the interpreter backend."""
        if log_fn is None:
            log_fn = self._append_cnc_log
        ...
        if not text:
            log_fn("[SYSTEM] Enter a CNC payload first.")
            return
    ...
    def _build_services_tab(self, parent):
        ...
        self.play_last_button = ttk.Button(provider_frame, text="Play Last", command=self.play_last_audio)
        self.play_last_button.grid(row=0, column=3, sticky="w", padx=6, pady=2)
    ...
    def _build_input_bar(self, parent):
        ...
        # Inline CNC tool sender from the main console
        ttk.Label(frame, text="CNC payload (JSON to tool):", style="TLabel").grid(row=1, column=0, sticky="w", padx=5, pady=(0, 2))
        self.cnc_console_input = tk.StringVar()
        cnc_entry = ttk.Entry(frame, textvariable=self.cnc_console_input, width=60)
        cnc_entry.grid(row=2, column=0, sticky="ew", padx=5, pady=(0, 5))
        cnc_entry.bind("<Return>", lambda e: self._send_cnc_custom(self.cnc_console_input.get(), log_fn=lambda m: self.append_chat("SYSTEM", m)))
        ttk.Button(
            frame,
            text="Send CNC (tool)",
            command=lambda: self._send_cnc_custom(self.cnc_console_input.get(), log_fn=lambda m: self.append_chat("SYSTEM", m)),
    ...
    def _populate_tts_voices(self):
        ...
        names = []
        display = []
        current = self.tts_manager.get_selected_voice() if hasattr(self, "tts_manager") else None
        for v in voices:
            name = v.get("name")
        ...
        if not names:
            ...
            # Try to preserve prior selection when still present; otherwise use first.
            if current and current in names:
                idx = names.index(current)
            else:
                idx = 0
            self.tts_voice_var.set(display[idx])
            self.tts_manager.set_voice(names[idx])
    ...
    def play_last_audio(self):
        """Play the most recently synthesized audio file, if available."""
        if not self.last_audio_path or not os.path.exists(self.last_audio_path):
            self._append_diag("[TTS] No saved audio to play.")
            return
        self._append_diag(f"[TTS] Playing {self.last_audio_path}")
    ...
    def _speak(self, text: str):
        ...
        if not audio:
            reason = getattr(self.tts_manager, "last_error", None)
            detail = f" Reason: {reason}" if reason else ""
            self._append_diag(f"[TTS] No audio produced (provider may be unavailable).{detail}")
            return
        try:
            base_dir = os.getenv("LOCALAPPDATA") or os.path.expanduser(r"~\\AppData\\Local")
            audio_dir = os.path.join(base_dir, "Jaden", "JL_Engine", "audio")
            os.makedirs(audio_dir, exist_ok=True)
            fname = os.path.join(
                audio_dir,
                f"tts_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{int(time.time()*1000)%1000}.mp3",
            )
    ...
    def _play_audio(self, path: str):
        """Play audio from a file path in a non-blocking thread."""
        try:
            from playsound import playsound
        except Exception as exc:
            self._append_diag(f"[TTS] Unable to import playsound: {exc}")
            return
        ...
        def _run():
            try:
                ...
                self._append_diag(f"[TTS] Playback failed: {exc}")
    ...
    def _update_linda_panel(self):
        ...
        if sig:
            hv["sentiment"].set(f"{sig.sentiment:.2f}")
            hv["arousal"].set(f"{sig.arousal:.2f}")
            hv["confusion"].set(f"{sig.confusion:.2f}")
            hv["pace"].set(f"{sig.pace:.2f}")
            hv["memory_density"].set(f"{sig.memory_density:.2f}")
            hv["directive"].set(str(bool(sig.directive)))
        else:
    ...
    def _append_diagnostics_snapshot(self):
        ...
        snapshot = {
            ...
            "signals": self.last_signals.__dict__ if self.last_signals else {},
        }
    ...
    def on_send(self, event=None):
        ...
        # Derive signals from user text (tone/pace/confusion) and keep trigger as secondary
        turn_signals = self.signal_scorer.score(user_text)
        self.last_signals = turn_signals
        signal_trigger = self._trigger_from_signals(turn_signals)
        keyword_trigger = self._detect_trigger(user_text)
        self.last_trigger = signal_trigger or keyword_trigger

        # Update gait/behavior/rhythm from the detected trigger
        self._update_gait_from_trigger(self.last_trigger, turn_signals)
        self._update_behavior_from_trigger(self.last_trigger)
        self._update_rhythm_from_trigger(self.last_trigger, turn_signals)

        # Recompute emotional aperture with the new state
        aperture_signal_overrides = self._aperture_signals_from_trigger(self.last_trigger)
        aperture_signal_overrides.update({
            "user_sentiment": turn_signals.sentiment,
            "conversation_pacing": turn_signals.pace,
            "memory_density": turn_signals.memory_density,
        })
        self.emotional_aperture.update_from_signals(
            ...
            aperture_bias=self.aperture_bias,
            safety_mode=self.safety_var.get() == "ON",
            **aperture_signal_overrides,
        )
        ...
        system_prompt = build_system_prompt(
            ...
            gait=self.current_gait,
            cognitive_mode=self.current_cognitive_mode,
        ...
        if command_text:
            dispatch_result = {}
            handled_cnc = self._maybe_dispatch_cnc_command(command_text)
            if not handled_cnc:
                if not self.tool_enabled:
